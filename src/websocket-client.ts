...
export class XClawWebSocketClient {
  ...
  private handleServerMessage(message: Record<string, unknown>) {
    ...
    // ✅ 调用所有注册的消息处理器
    // 实际处理逻辑在 plugin-dispatcher 中实现
    for (const handler of this.messageHandlers) {
      try {
        handler(message);
      } catch (err) {
        this.error(
          `[XClaw] 消息处理器执行失败: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
  }
   ...
   * 用于接收后端转发的用户消息和查询请求
   */
  addMessageHandler(handler: (msg: Record<string, unknown>) => void): void {
   ...
   */
  private toNativeChatEvent(event: AgentEventPayload): NativeChatEvent {
